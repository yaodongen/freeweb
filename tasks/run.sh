...
for path in `ls $FREEWEB_TASK_PATH/data/$PRE*.json`;do
    if [ $IS_SERVER == true ] ;then
        ...
        if [ -n "$success_load" ] ; then
            echo `date` : SUCCESS:$path,pid=$success_load >>$FREEWEB_TASK_PATH/../log/start_server.log 
            state=1
        else
            echo `date` : ERROR:$path >>$FREEWEB_TASK_PATH/../log/start_server.log 
            state=-1
        fi
...
if [ $1 -eq 0 ];then
    ...
    echo `date` : "flushed iptables ssserver6" >>$FREEWEB_TASK_PATH/../log/start_server.log 
elif [  $1 -eq 1 ] ; then
